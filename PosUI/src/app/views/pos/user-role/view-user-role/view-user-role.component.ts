...
export class ViewUserRoleComponent {
  ...
  uiInfo() {
    this.commonService.setUiInfo({ 
      title: 'View User Role List',
      goBackPath: '/pos/user-role', 
      refreshPath: '/pos/user-role/view',
      addNewPath: '/pos/user-role'
    })
  };
  ...
  updateRecord(element: any) {
    this.router.navigate(['/pos/user-role', { id: element.id }]);
  };

  deleteUserRole(element: any) {
    this.delSub =
    this.UserRoleService.deleteUserRole(element.id).subscribe((res: any) => {
