...
namespace PosWebAPIs.Controllers
{
    ...
    public class UserCategoryController : ControllerBase
    {
        ...
        public UserCategoryController(IUserCategoryService UserCategoryServices, ModelContext db)
        {
            _UserCategoryService = UserCategoryServices;
            _db = db;
        }
        ...
        public IActionResult ExecutePrecedure()
        {
            try
            {
                var data = _UserCategoryService.GetAll(_db);
        ...
        public IActionResult DuplicateCheck(UserCategory model)
        {
            try
            {
                var data = _UserCategoryService.DuplicateCheck(_db, model);
        ...
        [HttpPost("add/")]
        /*[Authorize(Policy = "OnlyNonBlockedCustomer")]*/
        public IActionResult Add(List<UserCategory> model)
        {
            try
            {
                var data = _UserCategoryService.Add(model, _db);
                if (data != null)
                {
                    returnObj.IsExecuted = true;
        ...
        //[Authorize(Policy = "OnlyNonBlockedCustomer")]
        public IActionResult GetUserCategoryById(int Id)
        {
            try
            {
                var data = _UserCategoryService.GetUserCategoryById(_db, Id);
                if (data != null)
                {
                    returnObj.IsExecuted = true;
                    returnObj.Data = data;
        ...
        [HttpPost("update/")]
        //[Authorize(Policy = "OnlyNonBlockedCustomer")]
        public IActionResult updateById(UserCategory model)
        {
            using (var dbTransaction = _db.Database.BeginTransaction())
            {
                try
                {
                    var data = _UserCategoryService.UpdateUserCategoryById(_db, model);
                    if (data)
                    {
        ...
        {
            var data = _UserCategoryService.DeleteUserCategory(_db, id);
            if (data)
            {
                returnObj.IsExecuted = true;
