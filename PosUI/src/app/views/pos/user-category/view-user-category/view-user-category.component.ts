...
@Component({
  selector: 'app-view-user-category',
  templateUrl: './view-user-category.component.html',
  styleUrls: ['./view-user-category.component.scss']
})
export class ViewUserCategoryComponent {
  ...
  uiInfo() {
    this.commonService.setUiInfo({ 
      title: 'View User Category List',
      goBackPath: '/pos/user-category', 
      refreshPath: '/pos/user-category/view',
      addNewPath: '/pos/user-category'
    })
  };
  ...
  updateRecord(element: any) {
    this.router.navigate(['/pos/user-category', { id: element.id }]);
  };

  deleteUserCategory(element: any) {
    this.delSub =
    this.userCategoryService.deleteUserCategory(element.id).subscribe((res: any) => {
  ...
  onDeleteConfirmation = (element: any): void => {
    this.commonService.showDialog(
      ...
      () => this.deleteUserCategory(element)
    );
  };
